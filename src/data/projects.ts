...
export const learningSites: LearningSite[] = [
  ...
  {
    slug: 'golden-whistle',
    name: '골든 휘슬 Golden Whistle',
    subject: '수학',
    category: '수학',
    tagline: '휘슬이 울리면 작전판을 편다 — 아는 만큼 작전이 통한다',
    description:
      '브라우저에서 도는 탑다운 2.5D 7인제 축구입니다. 조작은 버튼 하나가 두 가지 일을 합니다 — 공을 가지면 패스와 슛이고, 상대가 가지면 같은 버튼이 뺏기와 슬라이딩이 됩니다(화면의 버튼 이름도 그때그때 바뀝니다). 달리기를 «탭»하면 0.6초짜리 순간 대시가 터지는데, 체력을 쓰고 공을 치고 나가는 것이라 빠른 만큼 뺏길 여지도 생깁니다. 경기 전과 하프타임, 그리고 프리킥 직전에 작전판 문제가 한 개 뜹니다. 맞히면 그 경기 안에서만 쓰는 작전 카드가 생기고(승률 +10.5%p), 하나도 못 풀어도 경기는 100% 그대로 돌아가고 랭킹에도 오릅니다. 문제는 14초 안에 판단하는 «암산으로 닿는 수»만 냅니다 — 종이에 대고 풀어야 하는 계산은 한 문항도 없습니다.',
    features: [
      '7인제(GK 1 + 필드 6) · 전·후반 2분 30초 · 포메이션 3종 · 브로드캐스트 카메라 옵션',
      '문맥 버튼 — 공을 가지면 패스·슛, 없으면 뺏기·슬라이딩(모바일은 버튼 이름과 색이 함께 바뀝니다)',
      '순간 대시 — 달리기 탭 0.6초 버스트(걷기 대비 이동 +76%) · 체력 소모와 쿨다운이 있습니다',
      '슛은 길게 누를수록 세게(속도 14→33) · 머리 위 막대로 힘이 보입니다',
      '학습은 «상»으로만 — 맞히면 카드, 틀려도 벌 없음 · 학년·학기를 고르면 그 범위 문항만 나옵니다',
      '매일 자정 새로 시작하는 일일 리그 · 어제 1등은 오늘 고스트로 등장 · 이름은 가운데 글자를 지운 뒤에만 저장',
      '스프라이트 4,280장·효과음 12종·배경음 2곡을 전부 자체 생성(외부 조달 0건)',
    ],
    target: '초등 1~6학년(학년·학기 선택)',
    tech: ['TypeScript', 'Phaser 3', 'Cloudflare Pages/Functions/KV'],
    image: '/images/portfolio/golden-whistle.webp',
    url: 'https://golden-whistle.pages.dev/',
  },
  {
    slug: 'racket-rush',
